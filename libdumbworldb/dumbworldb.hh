...
namespace dumb::worldbase 
{
   ...
   public:
      ...
      Level();
      ~Level();
      // Copy an existing level structure.  Does not copy observers.
      // Might be useful for quickly restarting the level when the
      // player dies.
      Level(const Level &);
      // Load a level from a Doom-style WAD file.  The newly-loaded
      // level will contain things of all difficulty levels.  Call 
      // set_difficulty to get rid of everything else.
      void load_doom(std::string name);
      // Save the level to a Doom-style WAD file.
      // Index numbers are not retained.
      void save_doom(std::string filename) const;
      // Change the name of the level, like "E1M1".  Libdumbworldb
      // doesn't limit the length of the name but Doom-style WADs can
      // only store 8 characters.
      void set_name(std::string name);
      // Change the long name of the level, like "Hangar".  The length
      // of the name is unlimited.
      void set_longname(std::string longname);
      // Delete all things that shouldn't exist in DIFFICULTY.
      // This may only be called once per level.
      void set_difficulty(int difficulty);
      // Register an observer.  The Observer constructor calls this.
      void register_observer
   private:
      std::string name;		// "E1M1"
      std::string longname;	// "Hangar"
      std::list<Observer> observers;
   private:
      Level &operator =(const Level &); // not implemented
   };
   ...
   private:
      ...
      Texture floor_texture, ceiling_texture;
      int light;
      int type, tag;		// type and tag could be rethought
   ...
   {
   private:
      Side_texture upper, middle, lower;
      Sector *sector;
      Line *line;
   };
   ...
   public:
      enum {			// flags
	 F_IMPASSABLE  = 0x0001;
	 F_MIMPASSABLE = 0x0002;
	 F_SECRET      = 0x0004;
	 F_MBLKSOUND   = 0x0008;
	 F_NOMAP       = 0x0010;
	 F_MAPPED      = 0x0020; // set when player sees this line
      };
   private:
      Vertex *ver1, *ver2;
      int type, tag;
   ...
   public:
      enum {			// flags
	 F_NOCOLLISION    = 0x0001; // spispopd
	 F_PARTINVIS      = 0x0002; // partial invisibility
	 F_DEAF           = 0x0004; // doesn't react to sounds
	 F_IN_SKILL12     = 0x0400; // appears in skill levels 1-2
	 F_IN_SKILL3      = 0x0800; // appears in skill level 3
	 F_IN_SKILL45     = 0x1000; // appears in skill levels 4-5
	 F_IN_SINGLE      = 0x2000; // appears in single-player games
	 F_IN_COOP        = 0x4000; // appears in cooperative games
	 F_IN_DM          = 0x8000; // appears in DumbMatch games
      };
   protected:
      virtual ~Thing();
      // many more virtual functions are expected
   private:
      fixed x, y, z;
      fixed angle;		// radians
      fixed elev;
      Sector *center_sector;
      // FIXME: how to handle ThingSecs?
      unsigned flags;
   };
   ...
   {
   public:
      // The constructor hooks the observer to a level.  When the
      // level is deleted, the observer is notified but not
      // automatically destroyed.
      Observer(Level &);
      // The destructor does the opposite.
      virtual ~Observer();
      enum Phase {
	 AFTER_NEW,
	 AFTER_CHANGE,
	 BEFORE_DEL
      ...
      virtual void notice_level(const Level &, Phase);
      virtual void notice_sector(const Sector &, Phase);
      virtual void notice_side(const Side &, Phase);
      virtual void notice_sidetex(const Side_texture &, Phase);
      virtual void notice_line(const Line &, Phase);
      virtual void notice_vertex(const Vertex &, Phase);
      virtual void notice_thing(const Thing &, Phase);
   private:
      // The level to which this observer is hooked.  Used by the
      // destructor.  When the level is destroyed, this is set to
      // NULL.
      Level *level;
   };
} // namespace
