...
const void *safe_mmap(const char *name,int fd,unsigned int offset,size_t len) {
   ...
   if(p==(caddr_t)-1) {
      if(name) logprintf(LOG_FATAL,'S',"Error %d on mmap of %lu bytes for %s",errno,(unsigned long)len,name);
      else logprintf(LOG_FATAL,'S',"Error %d on mmap of %lu bytes",errno,(unsigned long)len);
   };
   return p;
#else
   ...
   if(lseek(fd,offset,SEEK_SET)==-1)
     logprintf(LOG_ERROR,'S',"Bad offset (%d) in map attempt for %s (errno=%d)",offset,name,errno);
   if((l=read(fd,p,len))<len)
     logprintf(LOG_ERROR,'S',"Bad length (%lu) in map attempt for %s (errno=%d) (got %lu)",(unsigned long)len,name,errno,(unsigned long)l);
   return p;
#endif
};
...
void safe_munmap(const char *name,const void *ptr,size_t len) {
   ...
   if(munmap((caddr_t)ptr,len)==-1) 
     logprintf(LOG_FATAL,'S',"Error %d munmapping %s",errno,name?name:"???");
#else
   safe_free((void *)ptr);
#endif
};
