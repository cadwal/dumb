	    ...
	    int xlace, int ylace)
{
  if ((xfact & yfact) == 1)
	...
	for (i=0; i < xsize; i += 4) {
	  ...
	  unsigned char register pix1=fb[i],pix2=fb[i+1],pix3=fb[i+2],pix4=fb[i+3];
	  tmp = ((unsigned long)pix1 + ((unsigned long)pix1 << 8)
		 + ((unsigned long)pix2 << 16) + ((unsigned long)pix2 << 24)
		 + ((unsigned long)pix3 << 32) + ((unsigned long)pix3 << 40)
		 + ((unsigned long)pix4 << 48) + ((unsigned long)pix4 << 56));
	...
	for (i=0; i < xsize; i += 2) {
	  ...
	  tmp = ((unsigned long)pix1 + ((unsigned long)pix1 << 8)
		 + ((unsigned long)pix1 << 16) + ((unsigned long)pix1 << 24)
		 + ((unsigned long)pix2 << 32) + ((unsigned long)pix2 << 40)
		 + ((unsigned long)pix2 << 48) + ((unsigned long)pix2 << 56));
	     ...
	     int xlace, int ylace)
{
  if ((xfact & yfact) == 1)
	...
	for (i=0; i < xsize; i += 2) {
	  ...
	  tmp = ((unsigned long)pix1 + ((unsigned long)pix1 << 16)
		 + ((unsigned long)pix2 << 32) + ((unsigned long)pix2 << 48));
	  *((unsigned long*)(rendfb+mul+width))
	    = *((unsigned long*)(rendfb+mul)) = tmp;
	}
	rendfb += rendinc;
	...
	for (i=0; i < xsize; i++) {
	  ...
	  tmp = ((unsigned long)pix1 + ((unsigned long)pix1 << 16)
		 + ((unsigned long)pix1 << 32) + ((unsigned long)pix1 << 48));
	  *((unsigned long*)(rendfb+mul+width*3))
	    = *((unsigned long*)(rendfb+mul+width*2))
	    = *((unsigned long*)(rendfb+mul+width))
	    = *((unsigned long*)(rendfb+mul)) = tmp;
