...
ConfItem input_conf[]={
   CONFB("no-mouse",NULL,0,"disable mouse support"),
   CONFE("mouse-type",NULL,0,"mouse type",0,mousies),
   CONFS("mouse-dev",NULL,0,"mouse device","/dev/mouse",254),
   {NULL}
};
...
static int mouse_dx=0, mouse_dy=0, mouse_buts=0, last_mx=0, last_my=0;

static void update_dumb_mouse(void) {
     int mouse_x, mouse_y;
     ...
     if (mouse_dx < -15000) mouse_dx += 30000;
     if (mouse_dx >  15000) mouse_dx -= 30000;
     if (mouse_dy < -15000) mouse_dy += 30000;
     if (mouse_dy >  15000) mouse_dy -= 30000;
}
...
void get_input(PlayerInput *in) {
   ...
   keyboard_update();
   if(!cnf_no_mouse) update_dumb_mouse();
   memset(in,0,sizeof(PlayerInput));

   /* shift */
   if(k_k(SCANCODE_LEFTSHIFT) || k_k(SCANCODE_RIGHTSHIFT)) speed=2*UNIT_SPEED;
...
void init_input(void) {
   if(keyboard_init())
     logprintf(LOG_FATAL,'I',"Console open failed");
   else
     logprintf(LOG_INFO,'I',"Console in raw mode");
