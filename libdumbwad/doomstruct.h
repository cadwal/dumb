 ...
 *
 * libdumbwad/doomstruct.h: Structures in a Doom WAD file.
...
#ifndef LIBDUMBWAD_DOOMSTRUCT_H
#define LIBDUMBWAD_DOOMSTRUCT_H
...
#ifdef __cplusplus
/* From the December 1996 C++ draft:

   "An object of a class with a non-trivial default constructor, a
   non-trivial copy constructor, a non-trivial destructor, or a
   non-trivial copy assignment operator cannot be a member of an
   union, nor can an array of such objects."

   "If there is no user-declared constructor for class X, a default
   constructor is implicitly declared.  [...]  A constructor is
   trivial if it is an implicitly-declared default constructor and if
   [...] for all the nonstatic data members of its class that are of
   class type (or array thereof), each such class has a trivial
   constructor."

   LE_int32 has a user-declared constructor which takes an int
   parameter, so it cannot have a trivial default constructor, nor can
   any class with a LE_int32 member.  Objects of such classes can't be
   union members.  Instead, we define each such member as a function
