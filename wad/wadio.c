...
static void load_wad(const char *fname,const char *_sig,int mapall) {
   ...
   /* load directory */
   if(wad->whole_map) wad->dir=WHOLEMAP_PTR(wad,wh->diroffset);
   else wad->dir=
     safe_mmap(fname,wad->fd,wh->diroffset,wh->nlumps*sizeof(WadDirEntry));
   wads=wad;
   nwads++;
			 ...
			 int lvl) 
{
   ...
   if(before) {
      ...
      if(!LUMPNUM_OK(l2)) logprintf(lvl,'W',"Can't find end-marker %s",before);
      else if(l2<l1) logprintf(lvl,'W',"End-marker %s occurs before lump %s",before,name); 
   }
   return l1;
};
