...
static void init_playpal(void) {
   if(playpal==NULL) {
      ppln=getlump("PLAYPAL");
      playpal=load_lump(ppln);
   }
}
static void reset_playpal(void) {
   if(playpal!=NULL) {
      free_lump(ppln);
      playpal=NULL;
   }
...
void set_playpal(int idx,SetPalFunc func) {
   int i=256;
...
static void paste_pict_on_texture(Texture *t,const PictData *p,int x,int y) {
   switch(t->bpp) {
    case(1): ppot8(t,p,x,y); break;
...
static void paste_jpatch_on_texture(Texture *t,const AltPictData *p,
				    int x,int y) {
   switch(t->bpp) {
    case(1): jpot8(t,p,x,y); break;
...
static void init_pnames(void) {
   ...
   free_lump(ln);
   logprintf(LOG_DEBUG,'T',"npnames=%d (should be %d)",npnames,
	     (get_lump_len(ln)-4)/8);
}
...
static void init_texture_lumps(void) {
   if(texture1==NULL) {
      t1ln=lookup_lump("TEXTURE1",NULL,NULL);
      t2ln=lookup_lump("TEXTURE2",NULL,NULL);
      if(t1ln==BAD_LUMPNUM)
	logprintf(LOG_FATAL,'T',"No metatexture lumps in WAD");
      texture1=load_lump(t1ln);
      if(t2ln==BAD_LUMPNUM) texture2=NULL;
      else texture2=load_lump(t2ln);
      nwalltexs=texture1->UMEMB(hdr).ntxts;
      logprintf(LOG_DEBUG,'T',"%s has %d wall textures","TEXTURE1",
		(int)(texture1->UMEMB(hdr).ntxts));
      if(texture2) {
	 nwalltexs+=texture2->UMEMB(hdr).ntxts;
	 logprintf(LOG_DEBUG,'T',"%s has %d wall textures","TEXTURE2",
		   (int)(texture2->UMEMB(hdr).ntxts));
      }
   }
}
static void reset_texture_lumps(void) {
   if(t1ln!=BAD_LUMPNUM) free_lump(t1ln);
   if(t2ln!=BAD_LUMPNUM) free_lump(t2ln);
   t1ln=t2ln=BAD_LUMPNUM;
   texture1=texture2=NULL;
   nwalltexs=0;
}

static const TextureData *find_tdata(int i) {
...
static void free_many_textures(Texture *tt,int i) {
   while(i>0) free_texels(tt+(--i));
}

static Texture *find_texture(const char *name,Texture *tt,int n) {
   int i;
   for(i=0;i<n;i++) 
     if(!strncasecmp(tt[i].name,name,8)) return tt+i;
   return NULL;
}
...
static void guess_sprite_size(Texture *t) {
   const PictData *pd;
   if(t->width>0) return;
...
static void init_sprites(void) {
   ...
   if(nsprites<=0) logprintf(LOG_FATAL,'T',"Can't find any sprites in WAD");
   ...
   while(topsprite<nsprites) {
	 ...
	 if(!strcasecmp(t->name,"S_END")) break;
	 if(get_lump_len(ln)<8) {
#ifdef TXDEBUG
	    logprintf(LOG_DEBUG,'T',"too short sprite: %s",t->name);
#endif	    
	    continue;
	 }
	 t->opaque=0;
   ...
   logprintf(LOG_INFO,'T',"Init %d sprites",nsprites=topsprite);
}
...
static void load_sprite(Texture *t,int bpp) {
   const PictData *pd;
   size_t l;
   if(!LUMPNUM_OK(t->lumpnum)) 
      logfatal('T',"Bad lumpnum for texture %s in load_sprite()",t->name);
   pd=load_lump(t->lumpnum);
...
static void free_sprite(Texture *t) {
   if(t->alloced_texels) safe_free(t->texels);
   else free_lump(t->lumpnum);
   t->texels=NULL;
   t->alloced_texels=0;
   t->bpp=0;
}
...
Texture *get_sprite_texture(const char *name) {
   ...
   if(!t) logfatal('T',"Can't find sprite texture %s",name);
   guess_sprite_size(t);
   return t;
}

Texture *get_sprite(const char *sp,char ph,char r,char *mptr)  {
   int i;
   Texture *t;
   char mirror=0;
   for(i=0,t=sprites;i<nsprites;i++,t++) {
      if(strncmp(t->name,sp,4)) continue;
      if(t->name[4]==ph&&t->name[5]=='0') break;
      if(t->name[6]==ph&&t->name[7]=='0') {mirror++;break;}
      if(t->name[4]==ph&&t->name[5]==r) break;
      if(t->name[6]==ph&&t->name[7]==r) {mirror++;break;}
   }
   if(i>=nsprites)
      return NULL;
   guess_sprite_size(t);
...
static void init_flats(void) {
   LumpNum ln=0;
   int topflat=0;
   if(flats) return;
   nflats=count_lumps_between("F_START","F_END");
   if(nflats<=0) logprintf(LOG_FATAL,'T',"Can't find any flats in WAD");
   ...
   while(topflat<nflats) {
	 ...
	 if(!LUMPNUM_OK(ln)) break;
	 get_lump_name(ln,t->name);
	 if(!strcasecmp(t->name,"F_END")) break;
	 t->type=TT_FLAT;
	 t->opaque=1;
	 ...
	 case(64*64):
	    ...
	    /* none of the above */
	 default:
	    logprintf(LOG_ERROR,'T', "funny length (%d) for flat %s",
		      i,t->name);
	 }
      }
   }
   logprintf(LOG_INFO,'T',"Init %d flats",nflats=topflat);
}
static void reset_flats(void) {
   if(flats) {
...
static void load_flat(Texture *t,int bpp) {
   t->bpp=bpp;
   if(bpp==1) t->texels=(void *)load_lump(t->lumpnum);
   else {
      ...
      while(i>0) {
	 i--;
	 if(bpp==2) ((unsigned short *)(t->texels))[i]=pix8topix16(data[i]);
	 else if(bpp==4) ((unsigned int *)(t->texels))[i]=pix8topix32(data[i]);
      }
      free_lump(t->lumpnum);
   }
}

static void free_flat(Texture *t) {
   if(t->texels) {
      if(t->bpp==1) free_lump(t->lumpnum);
      else safe_free(t->texels);
   }
   t->texels=NULL;
   t->bpp=0;
}
...
Texture *get_flat_texture(const char *name) {
   ...
   if(t==NULL) 
     logfatal('T',"Can't find flat texture %s",name);
   return t;
}
...
static void init_patches(void) {
   ...
   for(i=0;i<npnames;i++)
     patlns[i]=lookup_lump(lookup_pname(i),NULL,NULL);
}
static void reset_patches(void) {
   int i;
   /* 
   ...
   */
   if(!patlns) return;
   for(i=0;i<npnames;i++) 
     if(LUMPNUM_OK(patlns[i])) free_lump(patlns[i]);
   safe_free(patlns);
   safe_free(patlumps);
   patlns=NULL;
   patlumps=NULL;
...
static const PictData *get_patch_data(int num) {
   if(patlumps[num]==NULL)  {
      if(!LUMPNUM_OK(patlns[num])) 
	 logprintf(LOG_FATAL,'T',"Patchnum %d (%s) doesn't exist"
		   ,num,pnames+num*9);
      else {
#ifdef TXDEBUG
	 logprintf(LOG_DEBUG,'T',"loading patchnum %d (%s)"
		   ,num,pnames+num*9);
#endif
...
static void init_walltexs(void) {
   int i;
   init_texture_lumps();
   init_patches();
   if(walltexs) return;
#ifdef TXDEBUG
   logprintf(LOG_DEBUG,'T',"init_walltexs(): %d textures",nwalltexs);
#endif
...
static void reset_walltexs(void) {
   if(walltexs) {
...
Texture *get_wall_texture(const char *name) {
   Texture *t;
   if(*name=='&') return get_flat_texture(name+1);
   init_walltexs();
   t=find_texture(name,walltexs,nwalltexs);
   if(t==NULL) 
     logprintf(LOG_FATAL,'T',"Can't find wall texture %s",name);
   return t;
}

static void load_wtex(Texture *t,int bpp) {
   ...
   /* apparently something is wrong with this code, causing it to */
   /* crash on DEC Alphas.  I'm not sure what it is (josh) */
   if(bpp==1&&td->npatches==1&&td->patch[0].x==0&&td->patch[0].y==0) {
...
#ifdef TXDEBUG
	 logprintf(LOG_DEBUG,'T',"using one-patch kludge for %s",td->name);
#endif
	 return;
      }
   }
#endif   
#ifdef TXDEBUG
   logprintf(LOG_DEBUG,'T',"load_walltex %s at %d bpp",t->name,bpp);
   logprintf(LOG_DEBUG,'T',"l2w=%d l2h=%d",t->log2width,t->log2height);
#endif
   t->texels=safe_malloc(i=(bpp<<(t->log2width+t->log2height)));
   t->alloced_texels=1;
   memset(t->texels,-1,i);
#ifdef TXDEBUG
   logprintf(LOG_DEBUG,'T',"%s: %d patches",t->name,(int)(td->npatches));
#endif
   for(i=0;i<td->npatches;i++) {
	 ...
	 logfatal('T',"Can't make sense of patch #%d, texture %s",
		  (int)(td->patch[i].pnum),td->name);
   }
#ifdef TXDEBUG
   logprintf(LOG_DEBUG,'T',"%s: loaded",t->name);
#endif
}

static void free_wtex(Texture *t) {
   if(t->alloced_texels) safe_free(t->texels);
   t->texels=NULL;
   t->alloced_texels=0;
   t->bpp=0;
...
int init_font(const char *format,int nchars,int ofs) {
   int i;
   for(i=0;i<nfonts;i++) {
      if(!strncmp(fontname+i*MAXFONTNAME,format,MAXFONTNAME)) {
	 logprintf(LOG_DEBUG,'T',"found font %s already loaded",format);
	 return i;
      }
   }
   if(nfonts>=MAXFONTS) logfatal('T',"Too many fonts");
   ...
   logprintf(LOG_INFO,'T',"Loading font #%d (%s)",nfonts,format);
   for(i=0;i<nchars;i++) {
      ...
      if(LUMPNUM_OK(t->lumpnum)) guess_sprite_size(t);
   }
   /* done */
   return nfonts++;
}
...
Texture *get_font_texture(int fontnum,unsigned char _ch) {
   int ch=_ch-fontofs[fontnum];
   if(fontnum<0||fontnum>=nfonts) 
      logfatal('T',"Request for bad fontnum (%d)",fontnum);
   if(ch>=nfontents[fontnum]) 
      logfatal('T',"Request for bad font char (%d:%d)",fontnum,ch);
...
Texture *get_misc_texture(const char *name) {
   Texture *t;
   if(!misc) init_misc();
   t=find_texture(name,misc,nmisc);
   if(t) return t;
   if(nmisc>=MAXMISC) 
      logfatal('T',"too many misc textures");
   t=misc+nmisc++;
   t->type=TT_MISC;
   t->opaque=0;
...
int get_texture_num(TextureType tt,const char *name) {
   switch(tt) {
   case(TT_FLAT): return find_texture(name,flats,nflats)-flats; break;
   case(TT_WALL): return find_texture(name,walltexs,nwalltexs)-walltexs; break;
   case(TT_SPRITE): return find_texture(name,sprites,nsprites)-sprites; break;
   default: logprintf(LOG_ERROR,'T',
		      "get_texture_num: unsupported texture type (%d)",
		      (int)tt);
   }
   return 0;
}
Texture *get_texture_bynum(TextureType tt,int num) {
   switch(tt) {
    case(TT_FLAT): return flats+num;
    case(TT_WALL): return walltexs+num;
    case(TT_SPRITE): 
      guess_sprite_size(sprites+num);
      return sprites+num;
   default: logprintf(LOG_ERROR,'T',
		      "get_texture_bynum: unsupported texture type (%d)",
		      (int)tt);
   }
   return NULL;
}
int count_textures(TextureType tt) {
   switch(tt) {
    case(TT_FLAT): return nflats;
    case(TT_WALL): return nwalltexs;
    case(TT_SPRITE): return nsprites;
    case(TT_PATCH): return npnames;
    default: logprintf(LOG_ERROR,'T',
		       "count_textures: unsupported texture type (%d)",
		       (int)tt);
   }
   return 0;
}
...
void init_textures(void) {
   ...
   init_walltexs();
#ifdef TXDEBUG
   logprintf(LOG_DEBUG,'T',"init_textures(): done");
#endif
}
...
void free_texels(Texture *t) {
   if(t->texels==NULL) return;
   switch(t->type) {
    case(TT_FLAT): free_flat(t); break;
    case(TT_WALL): free_wtex(t); break;
    case(TT_MISC):
    case(TT_FONT):
    case(TT_SPRITE): free_sprite(t); break;
    default:
      logprintf(LOG_FATAL,'T',"Strange texture type (0x%x) in free_texels",t->type);
   }
}

void load_texels(Texture *t,int bpp) {
   /* some error checking */
   if(bpp!=1&&bpp!=2&&bpp!=4) 
     logprintf(LOG_FATAL,'T',"Strange BPP value (%d) in load_texels",bpp);
   if(t->texels&&t->bpp==bpp) return;
   if(t->texels) free_texels(t);
   /* now go do it */
   switch(t->type) {
    case(TT_FLAT): load_flat(t,bpp); break;
    case(TT_WALL): load_wtex(t,bpp); break;
    case(TT_MISC):
    case(TT_FONT):
    case(TT_SPRITE): load_sprite(t,bpp); break;
    default:
      logprintf(LOG_FATAL,'T',"Strange texture type (0x%x) in load_texels",
		t->type);
   }
   /* find average texel */
   switch(bpp) {
    case(1):t->avg_texel=*(unsigned char *)(t->texels);break;
    case(2):t->avg_texel=*(unsigned short *)(t->texels);break;
    case(4):t->avg_texel=*(unsigned int *)(t->texels);break;
   }
}
