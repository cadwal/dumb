...
/*CTLACTION(see_patches) {char buf[10];tchoose_patch(inst,buf);} */
CTLACTION(see_sprites)
{
   char buf[10];
   tchoose_sprite(inst, buf);
}
...
CTLACTION(load)
{
   load_instance(inst, inst->loadname);
}
...
CTLACTION(line_split)
{
   split_sel_lines(inst);
   RDRAW;
}

CTLACTION(line_crossing)
{
   cross_sel_lines(inst);
   RDRAW;
}

CTLACTION(mksector)
{
   make_sector_from_sel_lines(inst);
}

CTLACTION(connect_selected_vertices)
{
   connect_sel_vers(inst, 0);
}

CTLACTION(connect_selected_vertices_ccw)
{
   connect_sel_vers(inst, 1);
}

CTLACTION(join_lines)
{
   join_lines_at_vertex(inst, inst->curselect);
}

CTLACTION(mkcorridor)
{
   int s1 = inst->curselect, s2;
   for (s2 = 0; s2 < inst->nlines; s2++)
      if (s1 != s2 && (inst->enttbl[s2] & ENT_SELECTED))
	 break;
   if (s2 >= inst->nlines || s1 < 0)
      qmessage(inst, _("You must select exactly two lines."));
   else {
...
#define MS(n) CTLACTION(mkstairs##n) {\
...
CTLINPUT(inp_##field) {strcpy(inst->field,buf);rd;} \
CTLOUTPUT(out_##field) {strcpy(buf,inst->field);}

#define INST_NUM_INP_FUNX(field,rd) \
CTLINPUT(inp_##field) {inst->field=atoi(buf);rd;} \
CTLOUTPUT(out_##field) {sprintf(buf,"%d",(int)(inst->field));}

#define INST_FUNX(field,rd) \
CTLPRED(is_##field) {return inst->field;} \
CTLACTION(tog_##field) {inst->field=!inst->field;rd;}
...
#define MODEFUNX(x,y) \
CTLACTION(go##x##mode) {enter_mode(inst,y##Mode);} \
CTLPRED(is##x##mode) {return inst->mode==y##Mode;}
...
#define FUNX(x) \
CTLINPUT(sect_inp_##x) \
{if(VALID) CUR.x=atoi(buf); SEL(x=atoi(buf));} \
CTLOUTPUT(sect_out_##x) \
{if(VALID) sprintf(buf,"%d",(int)(CUR.x)); else *buf=0;}

#define FUNXT(x) \
CTLACTION(sect_act_##x) {if(VALID) \
...
#define FLFUNX(flag) \
CTLACTION(line_tog##flag) \
{if(VALID) {CURF^=LINE_##flag; \
if(CURF&LINE_##flag) {SEL(flags|=LINE_##flag);} \
else SEL(flags&=~LINE_##flag);}} \
CTLPRED(line_is##flag) {return VALID&&(CURF&LINE_##flag);}

#define FUNX(x,rd) \
CTLINPUT(line_inp_##x) \
{if(VALID) CUR.x=atoi(buf); SEL(x=atoi(buf)); rd;} \
CTLOUTPUT(line_out_##x) \
{if(VALID) sprintf(buf,"%d",(int)(CUR.x)); else *buf=0;}
...
CTLPRED(line_isTWOSIDED)
{
   return VALID && (CURF & LINE_TWOSIDED);
}
...
CTLINPUT(side##sn##_inp_id) {if(LINEVALID) SIDEID(sn)=atoi(buf);\
if(SIDEID(sn)>=inst->nsides) inst->nsides=SIDEID(sn)+1;} \
CTLOUTPUT(side##sn##_out_id) {if(LINEVALID) \
sprintf(buf,"%d",(int)(SIDEID(sn))); else *buf=0; }

#define SIDE_IO_SECT(sn) \
CTLINPUT(side##sn##_inp_sect) {if(SIDEVALID(sn)) \
SIDE(sn).sector=atoi(buf); SEL(sn,sector=atoi(buf));} \
CTLOUTPUT(side##sn##_out_sect) {if(SIDEVALID(sn)) \
...
{
   if (inst->scale < 0)
      sprintf(buf, "1:%d", 1 << -inst->scale);
   else
      sprintf(buf, "%d:1", 1 << inst->scale);
}
...
static const Control gen_ctls[GENROWS * GENCOLS] =
{
   ...
   LBUTTON(N_("BigMap"), tog_bigmap, is_bigmap, 0),
   LBUTTON(N_("VerMode"), govmode, isvmode, 0),

   IBUTTON(N_("Load:"), load, 0),
   INPUTCTL(inp_loadname, out_loadname, CTLF_USEFONT2),
   LBUTTON(N_("LineMode"), golmode, islmode, 0),

   IBUTTON(N_("Save As:"), save, 0),
   INPUTCTL(inp_mapname, out_mapname, CTLF_USEFONT2),
   LBUTTON(N_("SectMode"), gosmode, issmode, 0),

   IBUTTON(N_("Prev"), prev, 0),
   IBUTTON(N_("Next"), next, 0),
   LBUTTON(N_("ThingMode"), gotmode, istmode, 0),

   LBUTTON(N_("Grid:"), tog_showgrid, is_showgrid, 0),
   INPUTCTL(inp_gridsize, out_gridsize, 0),
   IBUTTON(N_("Sprites"), see_sprites, 0)
};
...
{
   IBUTTON(N_("Zoom In"), zoomi, 0),
   IBUTTON(N_("Zoom Out"), zoomo, 0),
   LABELCTL(N_("Scale:"), 0),
   OUTCTL(out_scale, 0)
};
...
{
   LABELCTL(N_("Vertex #"), 0),
   INPUTCTL(selectinp, selectout, 0),
   INPAIR(ver, N_("X-coord"), x, 0),
   INPAIR(ver, N_("Y-coord"), y, 0),
   IBUTTON(N_("Connect"), connect_selected_vertices, 0),
   IBUTTON(N_("CCW"), connect_selected_vertices_ccw, 0),
   IBUTTON(N_("JoinLines"), join_lines, 0),
   NULLCTL
};
...
static const Control sectmode_ctls[SECTROWS * SECTCOLS] =
{
   LABELCTL(N_("Sect #"), 0),
   INPUTCTL(selectinp, selectout, 0),
   INPAIR(sect, N_("FloorLvl"), floor, 0),
   INPAIR(sect, N_("CeilingLvl"), ceiling, 0),
   INBPAIR(sect, N_("Floor"), ftexture, CTLF_USEFONT2),
   INBPAIR(sect, N_("Ceiling"), ctexture, CTLF_USEFONT2),
   INPAIR(sect, N_("Light"), light, 0),
   INPAIR(sect, N_("Type"), type, 0),
   INPAIR(sect, N_("Tag"), tag, 0),
   IBUTTON(N_("Stairs"), mkstairs8, 0),
   IBUTTON(N_("BigStairs"), mkstairs16, 0)
};
...
static const Control thingmode_ctls[THROWS * THCOLS] =
{
   LABELCTL(N_("Thing #"), 0),
   INPUTCTL(selectinp, selectout, 0),
   INPAIR(th, N_("X-coord"), x, 0),
   INPAIR(th, N_("Y-coord"), y, 0),
   TFLAG_BUTTON(N_("Easy"), 12),
   IBUTTON(N_("Delete"), del_thing, CTLF_DANGER),
   TFLAG_BUTTON(N_("Medium"), 3),
   TFLAG_BUTTON(N_("Multi"), MULTI),
   TFLAG_BUTTON(N_("Hard"), 45),
   TFLAG_BUTTON(N_("Deaf"), DEAF),
   INPAIR(th, N_("Angle"), angle, 0),
   INPAIR(th, N_("Type"), type, 0)
};
...
static const Control linemode_ctls[LINEROWS * LINECOLS] =
{
   LABELCTL(N_("Line #"), 0),
   INPUTCTL(selectinp, selectout, 0),
   IBUTTON(N_("Delete"), del_line, CTLF_DANGER),

   INPAIR(line, N_("Tag"), tag, 0),
   IBUTTON(N_("VerFlip"), line_vflip, 0),

   INPAIR(line, N_("Type"), type, 0),
   IBUTTON(N_("SideFlip"), line_sflip, 0),

   LABELCTL(N_("Vertex #"), 0),
   LINE_ICTL(ver1),
   LINE_ICTL(ver2),

   LFLAG_BUTTON(N_("Impassible"), IMPASSIBLE),
   LFLAG_BUTTON(N_("TwoSided"), TWOSIDED),
   LFLAG_BUTTON(N_("M-Impass"), MIMPASSIBLE),

   LFLAG_BUTTON(N_("UnpUpper"), UPUNPEG),
   LFLAG_BUTTON(N_("UnpLower"), LOUNPEG),
   LFLAG_BUTTON(N_("Secret"), SECRET),

   LFLAG_BUTTON(N_("BlockSnd"), BLKSOUND),
   LFLAG_BUTTON(N_("NoMap"), NOMAP),
   LFLAG_BUTTON(N_("ForceMap"), FORCEMAP),

   LFLAG_BUTTON(N_("Poster"), POSTER),
   NULLCTL,
   IBUTTON(N_("Crossing"), line_crossing, 0),

   IBUTTON(N_("Split"), line_split, 0),
   IBUTTON(N_("MkSector"), mksector, 0),
   IBUTTON(N_("Corridor"), mkcorridor, 0),

   SIDE_BUTTONS(N_("Side #"), id, 0),
   SIDE_BUTTONS(N_("Sector #"), sect, 0),
   SIDE_BUTTONS(N_("Offset"), ofs, 0),
   SIDE_BUTTONSB(N_("Texture"), texture, CTLF_USEFONT2),
   SIDE_BUTTONSB(N_("U-Texture"), utexture, CTLF_USEFONT2),
   SIDE_BUTTONSB(N_("L-Texture"), ltexture, CTLF_USEFONT2)
};


/* control sets (only these need be non-static) */

const ControlSet gen_cset[1] =
{
   {GENCOLS, GENROWS, gen_ctls}};
const ControlSet map_cset[1] =
{
   {MAPCOLS, MAPROWS, map_ctls}};
