 ...
 *
 * xwad/connect.c: Creating new sectors, lines, etc. in fancy ways.
...
mkside(XWadInstance *inst)
{
   ...
   static const struct dumblevel_side modelside =
   {
      ...
      {NULL, 0, 0, 0x0000},	/* middle */
      {NULL, 0, 0, 0x0000},	/* lower */
      -1,			/* sector */
      -1, -1
   };
...
cpyline(XWadInstance *inst, int o)
{
   ...
   int i;
   for (i = 0; i < 2; i++) {
      if (modelline.side[i] != -1)
	 modelline.side[i] = dumblevel_new_side(&inst->level,
						&modelline.side[i]);
   }
   return dumblevel_new_line(&inst->level, &modelline);
#else  /* !DUMB_CONFIG_LDWB */
...
cross_sel_lines(XWadInstance *inst)
{
   ...
   if (!cross_lines(inst, l[0], l[1]))
      qmessage(inst, _("Lines don't cross."));
}

void
connect_sel_vers(XWadInstance *inst, int ccw)
{
   double a = 0.0;
   int i, v, first_line;
   int concave = 0;
   ...
   if (inst->curselect < 0 || inst->curselect >= inst->nvers)
      return;
   ...
   //logprintf(LOG_DEBUG, 'C', _("first vertex is %d, angle %f"), v, a);
   ...
   do {
	 ...
	 while (d < 0.0)
	    d += 2.0 * M_PI;
	 //logprintf(LOG_DEBUG, 'C', _("  vertex %d, angle is %f"), j, d);
	 if (d > md) {
	    ma = na;
	    md = d;
	    mv = j;
	 ...
	 if ((L(j).ver1 == i && L(j).ver2 == mv)
	     || (L(j).ver1 == mv && L(j).ver2 == i))
	    break;
      /* connect i -> mv */
      if (j == inst->nlines) {
	 logprintf(LOG_DEBUG, 'C', _("connecting %d -> %d"), i, mv);
	 l = mkline(inst);
	 L(l).ver1 = i;
	 L(l).ver2 = mv;
      } else
	 logprintf(LOG_DEBUG, 'C', _("%d and %d are already connected"),
		   i, mv);
   ...
   /* check for concavity */
   for (i = 0; i < inst->nvers; i++)
      if ((inst->enttbl[i] & ENT_SELECTED) && !touched[i])
	 concave = 1;
   /* go to line mode */
   enter_mode(inst, LineMode);
   for (i = first_line; i < inst->nlines; i++)
      new_selection(i, inst, 1);
   /* clean up */
   safe_free(touched);
   if (first_line == inst->nlines)
      qmessage(inst, _("No vertices were connected."));
   else if (concave)
      qmessage(inst, _("Some vertices were not connected."));
}
...
join_lines_at_vertex(XWadInstance *inst, int joinver)
{
   ...
   if (find_lines_at_vertex(inst, joinver, l, 2) != 2) {
      qmessage(inst, _("There must be exactly two lines connected to"
		       " the selected vertex."));
      return;
   }
   for (i = 0; i < 2; i++) {
   ...
   if (side[0] == -1)
      side[0] = altside[1];	/* which may be -1 too */
   if (side[1] == -1)
      side[1] = altside[0];
...
void
make_sector_from_sel_lines(XWadInstance *inst)
{
   int i = inst->curselect;
   int side = 0, sect;
   /* make sure at least one is selected */
   if (i < 0 || i >= inst->nlines)
      return;
   inst->enttbl[i] |= ENT_SELECTED;
   /* internal or external? */
   if (L(i).side[0] >= 0 && S(i, 0).sector >= 0)
      side = 1;
   /* make the sector */
   sect = mksect(inst);
   ...
   for (i = 0; i < inst->nlines; i++) {
	 ...
	 continue;
      if (L(i).side[side] < 0)
	 L(i).side[side] = mkside(inst);
      S(i, side).sector = sect;
      if (side)
	 L(i).flags |= LINE_TWOSIDED;
   }
   /* go to sector mode */
...
int
make_corridor_between(XWadInstance *inst, int l1, int l2)
{
   int s1, s2;
   int csect, l3, l4;
   s1 = S(l1, 0).sector;
   ...
   if (S(l1, 0).sector < 0
       ...
       || L(l1).side[1] >= 0
       || L(l2).side[1] >= 0) {
      qmessage(inst, _("You must select two one-sided sector boundaries."));
      return -1;
   }
   /* build corridor */
...
static int
get_corridor(XWadInstance *inst, int sect,
...
void
make_stairs(XWadInstance *inst, int sect, int inc)
{
   int p[2], w[2];
   int i, nstairs, lw0, lw1, lsect;
   int w0v1, w0v2, w1v1, w1v2;
   if (get_corridor(inst, sect, p, w)) {
      qmessage(inst, _("Sector does not seem corridor-like."));
      return;
   }
