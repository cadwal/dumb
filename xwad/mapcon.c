 ...
 *
 * xwad/mapcon.c: Selecting and moving map objects.
...
static int
nearest_vertex(int x, int y, const XWadInstance *inst)
{
   int i, j = -1;
   float min = BIGFLOAT;
...
static int
nearest_thing(int x, int y, const XWadInstance *inst)
{
   int i, j = -1;
   float min = BIGFLOAT;
...
static double
line_dist(const XWadInstance *inst, int line, int x, int y)
{
...
static int
nearest_line(int x, int y, const XWadInstance *inst)
{
   int i, j = -1;
   float min = BIGFLOAT;
...
static int
nearest_sector(int x, int y, const XWadInstance *inst)
{
...
void
new_entity(int x, int y, XWadInstance *inst, int extend)
{
   ...
   case (ThingMode):
      ...
      break;
   default:
      message(inst, _("No Create function for this mode yet!"));
      break;
   }
...
extern inline int
ver_selected_for_move(XWadInstance *inst, int v)
{
   switch (inst->mode) {
   case (VerMode):
...
void
move_entity(int x, int y, XWadInstance *inst)
{
   int i;
   ...
   if (inst->mode == ThingMode)
      for (i = 0; i < inst->nthings; i++) {
	 if (i != inst->curselect && !(inst->enttbl[i] & ENT_SELECTED))
	    continue;
	 inst->thing[i].x -= x;
	 inst->thing[i].y -= y;
   } else
      for (i = 0; i < inst->nvers; i++) {
	 if (!ver_selected_for_move(inst, i))
	    continue;
	 if (do_adjust) {
...
static void
selection_by_mouse(int x, int y, XWadInstance *inst, int ext)
{
   switch (inst->mode) {
   case (ThingMode):
      new_selection(nearest_thing(x, y, inst), inst, ext);
      break;
   case (VerMode):
      new_selection(nearest_vertex(x, y, inst), inst, ext);
      break;
   case (LineMode):
      new_selection(nearest_line(x, y, inst), inst, ext);
      break;
   case (SectMode):
      new_selection(nearest_sector(x, y, inst), inst, ext);
      break;
   }
}

static void
drag_start(XWadInstance *inst, int x, int y, DragMode mode)
{
   inst->dragstartx = x;
   inst->dragstarty = y;
   inst->dragmode = mode;
   switch (mode) {
   case (DragObj):
      XDefineCursor(dpy, inst->map, drag_obj_cursor);
      break;
   case (DragMap):
      XDefineCursor(dpy, inst->map, drag_map_cursor);
      break;
   case (NoDrag):
      break;
   }
...
static void
drag_update(XWadInstance *inst, int x, int y)
{
   ...
   case (DragObj):
      if (inst->showgrid
	  && abs(mapdx) < inst->gridsize
	  && abs(mapdy) < inst->gridsize)
...
static void
drag_end(XWadInstance *inst, int x, int y)
{
...
void
map_button(XButtonEvent *ev, XWadInstance *inst)
{
...
void
map_motion(XMotionEvent *ev, XWadInstance *inst)
{
