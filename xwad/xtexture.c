...
void xtexture(Display *dpy,Drawable d,Texture *t,int do_mirror) {
   ...
   XGetGeometry(dpy,d,&frame_root,
		...
		&border,&depth);
   if(width>t->width) xo=(width-t->width)/2;
   if(height>t->height) yo=(height-t->height)/2;
   if(do_mirror) do_mirror=1;
   bypp=depth/8;
   if(bypp==3) bypp++;
   if(t->xpix[do_mirror]==0) {
      ...
      image=XCreateImage(dpy,
			 ...
			 t->width,t->height,
			 8,bypp*t->width);
      if(do_mirror) switch(bypp) {
      case(1): mirror(signed char); break;
      case(2): mirror(signed short); break;
      case(4): mirror(signed int); break;
      } 
      else switch(bypp) {
