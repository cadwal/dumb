 ...
 *
 * xwad/controls.c: Buttons and other widgets.
...
void
get_cset_size(const ControlSet *cset, int *width, int *height)
{
...
static void
draw_text(Window w, int x, int y, const char *text, int center)
{
   XCharStruct xcs;
   int dirn, asc, dsc;
   int len = strlen(text);
...
static void
draw_ctllight(const Control *ctl, const CtlInstance *ci,
	      int col, int row,
	      Window w, AppInst *inst)
{
   int xo = col * CTLWIDTH, yo = row * CTLHEIGHT;
   if (ci->pressed && controls_3d) {
      xo++;
      yo++;
   }
...
static void
redraw_control(int col, int row, Window w)
{
...
void
draw_control(const Control *ctl, CtlInstance *ci,
	     int col, int row,
	     Window w, AppInst *inst,
	     int clear)
{
   ...
   if (ctl->f & CTLF_HASBUF) {
      if (ctl->output && !ci->editing)
	 ctl->output(ci->buf, inst);
      if (ci->pressed)
...
static void
ctl_edit(CSetInstance *csi, int i,
...
static void
ctl_edit_back(CSetInstance *csi, int i,
	      AppInst *inst)
{
   //const Control *ctl=csi->cset->ctls+i;
   CtlInstance *ci = csi->inst + i;
...
static void
ctl_edit_done(CSetInstance *csi, int i,
	      AppInst *inst, int act)
{
   ...
   if (ci->editing) {
      if (act && ctl->input)
	 ctl->input(ci->buf, inst);
      ci->editing = 0;
   }
}

void
release_buttons(CSetInstance *csi, AppInst *inst, int act)
{
   int row, col;
   for (row = 0; row < csi->cset->rows; row++)
      for (col = 0; col < csi->cset->cols; col++) {
	 const Control *ctl;
	 CtlInstance *ci;
	 ...
	 if (want_redraw)
	    redraw_control(col, row,
			   csi->w);
      }
}

void
rdlights_cseti(CSetInstance *csi, AppInst *inst)
{
   int col, row, i;
   for (row = 0; row < csi->cset->rows; row++)
      for (col = 0; col < csi->cset->cols; col++) {
...
void
rdoutp_cseti(CSetInstance *csi, AppInst *inst)
{
   int col, row, i;
   for (row = 0; row < csi->cset->rows; row++)
      for (col = 0; col < csi->cset->cols; col++) {
...
void
rdable_cseti(CSetInstance *csi, AppInst *inst)
{
   int col, row, i;
   for (row = 0; row < csi->cset->rows; row++)
      for (col = 0; col < csi->cset->cols; col++) {
...
void
expose_cseti(CSetInstance *csi, XExposeEvent *ev, AppInst *inst)
{
...
void
motev_cseti(CSetInstance *csi, XMotionEvent *ev, AppInst *inst)
{
}

void
butev_cseti(CSetInstance *csi, XButtonEvent *ev, AppInst *inst)
{
   int col, row, i;
   const Control *ctl;
   CtlInstance *ci;
   ...
   ctl = csi->cset->ctls + i;
   ci = csi->inst + i;
   if (col < 0 || col >= csi->cset->cols)
      return;
   if (row < 0 || row >= csi->cset->rows)
      return;
   /* deal with event */
...
void
keyev_cseti(CSetInstance *csi, XKeyEvent *ev, AppInst *inst)
{
   int col, row, i;
   const Control *ctl;
   CtlInstance *ci;
   ...
   if (ev->type == KeyPress && ctl->f & CTLF_HASBUF && ctl->input) {
      ...
      /* deal with it */
      if (ks == XK_Return)
	 ctl_edit_done(csi, i, inst, 1);
      else if (ks == XK_Escape)
	 ctl_edit_done(csi, i, inst, 0);
      else if (ks == XK_BackSpace)
	 ctl_edit_back(csi, i, inst);
      else if (ks == XK_Delete)
	 ctl_edit_back(csi, i, inst);
      else if (nchars > 0)
	 ctl_edit(csi, i, inst, str);
      draw_control(ctl, ci,
		   col, row,
		   csi->w, inst, 1);
   }
...
void
crossev_cseti(CSetInstance *csi, XCrossingEvent *ev, AppInst *inst)
{
   if (ev->type == LeaveNotify)
      release_buttons(csi, inst, 0);
}
