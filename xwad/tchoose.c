 ...
 *
 * xwad/tchoose.c: Texture chooser dialog.
...
static int
find(const char *prefix, AppInst *inst)
{
   return 0;
}

static void
chcur(int item, AppInst *inst)
{
   XClearArea(dpy, inst->tch_preview, 0, 0, 0, 0, True);
}

static void
choose(int item, AppInst *inst)
{
   Texture *t;
   if (item >= 0) {
...
setwmstuff(XWadInstance *inst, int set)
{
   ...
   if (set)
      XResizeWindow(dpy, inst->tch_frame,
		    size_hints.min_width, size_hints.min_height);
}

static void
chksize(AppInst *inst)
{
...
static void
framedhf(XEvent *ev, AppInst *inst, void *info)
{
   switch (ev->type) {
   case (KeyPress):
      {
...
static void
prevdhf(XEvent *ev, AppInst *inst, void *info)
{
...
CTLACTION(act_ok)
{
   choose_cur(&inst->tch, inst->tch.curitem, inst);
}

CTLACTION(act_cancel)
{
   choose_cur(&inst->tch, -1, inst);
}
...
CTLPRED(is_prev)
{
   return inst->tch_do_preview;
}

static const Control tch_ctl[3] =
{
   IBUTTON(N_("OK!"), act_ok, 0),
   IBUTTON(N_("Cancel"), act_cancel, CTLF_DANGER),
   LBUTTON(N_("Preview"), tog_prev, is_prev, 0)
};

static const ControlSet tch_ctls = {3, 1, tch_ctl};

void
init_tchoose(XWadInstance *inst)
{
   /* init textures */
   init_textures();
...
void
do_tchoose(XWadInstance *inst, TextureType tt, char *buf)
{
   inst->tch_buf = buf;
   inst->tch_type = tt;
   inst->tch.nitems = count_textures(tt);
   if (buf)
      choose_setcur(&inst->tch, get_texture_num(tt, buf), inst);
   setwmstuff(inst, 0);
   XMapRaised(dpy, inst->tch_frame);
}

void
tchoose_wall(XWadInstance *inst, char *buf)
{
   do_tchoose(inst, TT_WALL, buf);
}

void
tchoose_flat(XWadInstance *inst, char *buf)
{
   do_tchoose(inst, TT_FLAT, buf);
}

void
tchoose_sprite(XWadInstance *inst, char *buf)
{
   do_tchoose(inst, TT_SPRITE, buf);
}

void
tchoose_patch(XWadInstance *inst, char *buf)
{
   do_tchoose(inst, TT_PATCH, buf);
}

void
free_tchoose(XWadInstance *inst)
{
