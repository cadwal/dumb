 ...
 *
 * xwad/savemap.c: Saving the map to disk.
...
extern inline int
IsLineDefInside(int x1, int y1, int x2, int y2,
...
static void
wrblkmap(WADWR *w, const XWadInstance *inst)
{
...
gc_vers(XWadInstance *inst)
{
   ...
   for (i = 0; i < n; i++) {
      if (tbl[i] > 0) {
	 if (j < i)
	    memcpy(inst->ver + j, inst->ver + i, sizeof(VertexData));
	 tbl[i] = j++;
      } else
	 tbl[i] = -1;
   }
...
gc_sides(XWadInstance *inst)
{
   ...
   for (i = 0; i < n; i++) {
      if (tbl[i] > 0) {
	 if (j < i)
	    memcpy(inst->side + j, inst->side + i, sizeof(SideData));
	 tbl[i] = j++;
      } else
	 tbl[i] = -1;
   }
...
gc_sects(XWadInstance *inst)
{
   ...
   for (i = 0; i < n; i++) {
      if (tbl[i] > 0) {
	 if (j < i)
	    memcpy(inst->sect + j, inst->sect + i, sizeof(SectorData));
	 tbl[i] = j++;
      } else
	 tbl[i] = -1;
   }
...
void
garbage_collect(XWadInstance *inst)
{
   /*
