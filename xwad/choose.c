...
static void
movescroll(ChooseInst *ci, int i)
{
   ci->scroll += i;
   if (ci->scroll < 0)
      ci->scroll = 0;
...
static void
myrect(Drawable d, GC gc, int x, int y, int dx, int dy)
{
...
static void
thumbshape(ChooseInst *ci, int *start, int *end)
{
