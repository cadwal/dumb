 ...
 *
 * tool/todumb.c: Convert images to DUMB formats.
...
static const char *playpal_fn = "data/PLAYPAL.lump";
static unsigned char playpal[256 * 3];
...
print_help(void)
{
   print_converter_usage(argv0);
   printf(_("  -f, --flat            create a 64x64 flat-format graphic\n"
...
{
   Image *image = (Image *) xmalloc(sizeof(Image));
   int y;
   image->width = width;
   image->height = height;
