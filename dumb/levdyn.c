 ...
 *
 * dumb/levdyn.c: Initializing dynaming parts of a level.  Creating things.
...
#include "animtex.h"
#include "game.h"
#include "things.h"		/* thingd_findsector() */
...
LD_DYNDECL(Line)
{
   ...
   if (lump->side[0] >= 0)
      ldsided(ld)[lump->side[0]].lined = dyn;
   /*logprintf(LOG_DEBUG, 'M',
      _("initline: line %d length=%f (%f,%f)-(%f,%f)"),
      lump-ldline(ld),
...
int
find_empty_thing(const LevData *ld)
{
   static int i = 0;
   int last = i;
   do {
...
int
safe_find_empty_thing(const LevData *ld)
{
   int i = find_empty_thing(ld);
   if (i < 0)
      logprintf(LOG_FATAL, 'O', _("Ran out of thing slots"));
   return i;
}

int
new_thing(const LevData *ld, int prid, fixed x, fixed y, fixed z)
{
   ...
   if (prid > 0 && td->proto == NULL)
      logfatal('O', _("Couldn't find protothing ID=%d"), prid);
...
diffchk(const ThingData *t, const LevData *ld)
{
   ...
   logprintf(LOG_ERROR, 'O', _("diffchk: strange difficulty?\?\? (%d)"),
	     ld->difficulty);
   return 0;
}
...
LD_DYNDECL(Thing)
{
   ...
   /*logprintf(LOG_DEBUG, 'O', _("init thing type=%d at (%d,%d)"),
      lump->type, lump->x, lump->y); */
   dyn->x = MAPORD_TO_RENDERORD(lump->x);
   dyn->y = MAPORD_TO_RENDERORD(lump->y);
   dyn->z = 0;
