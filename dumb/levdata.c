 ...
 *
 * dumb/levdata.c: Loading and updating levels.
...
char *
get_next_level_name(char *buf, const char *n)
{
   if (n == NULL || *n == 0) {
...
void
init_player(LevData *ld, int pl, int th)
{
   ld->player[pl] = th;
...
void
load_level(LevData *ld, const char *name, int d, int mp)
{
   int i;
   logprintf(LOG_INFO, 'M', _("Loading %s: difficulty=%d"), name, d);
   ...
   for (i = 0; i < ML_NTYPES; i++) {
	 ...
	 ld->dyn_bkup[i] = safe_calloc(count + mlti[i].dynextra,
				       mlti[i].dyngrain);
      }
      if (mlti[i].dyninit) {
	 int j;
...
#ifdef LEVDATA_DEBUG
	 logprintf(LOG_DEBUG, 'M', _("Init %d %s"), count, mlti[i].lumpname);
#endif
	 for (j = 0; j < count; j++) {
   ...
   if (ld->player[0] < 0)
      logprintf(LOG_ERROR, 'M', _("Uh-oh: no player!"));
   /*logprintf(LOG_DEBUG, 'M', _("Done: %s"), name); */
}

void
reset_level(LevData *ld)
{
   int i;
   for (i = 0; i < ML_NTYPES; i++) {
	 ...
	 safe_free(ld->dyn[i]);
      if (ld->dyn_bkup[i])
	 safe_free(ld->dyn_bkup[i]);
   }
}
void
free_level(LevData *ld)
{
   int i;
#ifdef LEVDATA_DEBUG
   logprintf(LOG_INFO, 'M', _("Freeing level data for %s"), ld->name);
#endif
   for (i = 0; i < MAXPLAYERS; i++) {
      if (ld->plinfo[i])
	 safe_free(ld->plinfo[i]);
      if (ld->plbkup[i])
	 safe_free(ld->plbkup[i]);
   }
   reset_level(ld);
}

void
free_levlumptype(LevData *ld, MapLumpType mlt)
{
#ifdef LEVDATA_DEBUG
   logprintf(LOG_DEBUG, 'M', _("Pre-freeing %s for %s"),
...
generate_updates(LevData *ld)
{
   ...
   for (mlt = 0; mlt < ML_NTYPES; mlt++) {
      ...
      if (old == NULL || mlti[mlt].encode == NULL)
	 continue;
...
void
apply_update(LevData *ld, MapLumpType mlt, int ofs,
	     const void *code, int codelen)
{
   ...
   if (mlt < 0 || mlt >= ML_NTYPES)
      logfatal('M', _("bad maplumptype (%d) in apply_update()"), (int) mlt);
...
void
apply_plinfo_update(LevData *ld, int player, int offset, int value)
{
   if (!got_slave_info)
      return;
   if (ld->plinfo[player])
      ...
      logprintf(LOG_ERROR, 'M',
		_("got plinfo_update for player with no plinfo?"));
}
