 ...
 *
 * dumb/ggi_video.c: GGI input driver.
 * Copyright (C) 1998 by Andrew Apted <ajapted@ggi-project.org>
 *
...
init_video(int *width, int *height, int *bpp, int *real_width)
{
   ...
   if (err) {
      logfatal('V', _("GGI: SetGraphMode failed."));
   }
...
static void
handle_ggi_key_event(ggi_event * ev, PlayerInput *in)
{
   int state;
   int sym;
   ...
   if ((KTYP(sym) == KT_LATIN) || (KTYP(sym) == KT_LETTER) ||
	 ...
	 /* continuous actions */
      ...
      case 'H' - 64:
      case 'H':
	 MODSTATE(STATE_LEFT, state);
	 break;

      case 'l':
      case 'L' - 64:
      case 'L':
	 MODSTATE(STATE_RIGHT, state);
	 break;
	 ...
	 break;
      ...
      case ',':
      case '<':
	 MODSTATE(STATE_SIDELEFT, state);
	 break;

      case '.':
      case '>':
	 MODSTATE(STATE_SIDERIGHT, state);
	 break;
      ...
      case 'A' - 64:
      case 'A':
	 MODSTATE(STATE_UP, state);
	 break;

      case 'z':
      case 'Z' - 64:
      case 'Z':
	 MODSTATE(STATE_DOWN, state);
	 break;

      case 'd':
      case 'D' - 64:
      case 'D':
	 MODSTATE(STATE_LOOKUP, state);
	 break;

      case 'c':
      case 'C' - 64:
      case 'C':
	 MODSTATE(STATE_LOOKDOWN, state);
	 break;
#else  /* !KALLE_KEYS */
      case 'a':
      case '\001':
	 ...
	 /* modifiers */
      ...
      case K_NORMAL_SHIFT:
	 MODSTATE(STATE_SHIFT, state);
	 break;
#endif
#ifdef K_NORMAL_ALT
      case K_NORMAL_ALT:
	 MODSTATE(STATE_ALT, state);
	 break;
#endif
...
#ifdef K_NORMAL_CTRL
      case K_NORMAL_CTRL:
	 MODSTATE(STATE_CTRL, state);
	 break;
#endif
#endif /* !KALLE_KEYS */
      default:
	 printf(_("Unused key: Special "
		  "sym=%04x\n"), sym);
      }
   }
}
...
static void
handle_ggi_ptr_event(ggi_event * ev, PlayerInput *in)
{
   int state;
...
static void
update_continuous_actions(PlayerInput *in)
{
#define SPEEDOF(flag)  ((GSI.kb_state & (flag))		\
                        ? ((GSI.kb_state & STATE_SHIFT)	\
                           ? RUN_SPEED : UNIT_SPEED)	\
                        : 0)
