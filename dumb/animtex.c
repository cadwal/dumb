 ...
 *
 * dumb/animtex.c: Animated textures.
...
int
update_anim_state(AnimTexNum num, int snum, int tickspassed)
{
   int tnum = snum & 0xffff;
   if (!attd)
      init_animtex();
   if (num >= natt || num < 0)
      logfatal('A', _("Bad AnimTexNum (%d)"), num);
   num -= att[num].myseqnum;
   if (att[num].flags & AT_SWITCH)
      return snum;
   snum >>= 16;
   tnum += tickspassed;
   if (tnum >= att[num + snum].duration) {
      tnum = 0;
      snum++;
   }
   if (snum >= att[num].seqlen)
      snum = 0;
   return (snum << 16) | (tnum & 0xffff);
}

Texture *
get_anim_texture(AnimTexNum num, int snum)
{
   if (!attd)
      init_animtex();
   if (num >= natt || num < 0)
   ...
   num += snum;
   if (attd[num] == NULL) {
      if (att[num].flags & AT_FLAT)
	 attd[num] = get_flat_texture(att[num].name);
      else
	 attd[num] = get_wall_texture(att[num].name);
   }
   return attd[num];
}
