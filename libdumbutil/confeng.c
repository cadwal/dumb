 ...
 *
 * libdumb/confeng.c: The configuration engine.
...
lookup_etype(const ConfEnum *ce, const char *s)
{
   int i;
   for (i = 0; ce[i].name; i++)
      if (!strcasecmp(ce[i].name, s))
	 return i;
   /* FIXME: halt the program if given from cmdline */
   /* FIXME: stderr */
   printf(_("`%s' is not an option.  I'll use `%s' instead.\n"),
	  s, ce[0].name);
   return 0;
}
...
set_conf(ConfItem *ci, char *val, int dirt)
{
   ...
   case CONF_TYPE_INT:
	 ...
	 fprintf(stderr, _("switch requires an argument -- %s\n"),
		 ci->name);
   ...
   case CONF_TYPE_LIST:
	 ...
	 if (val)
	    ci->listval[0] = strdup(val);
      } else {
	 int n = 0;
	 while (ci->listval[n])
	    n++;
	 ci->listval = (char **) realloc(ci->listval,
					 (n + 2) * sizeof(char *));
	 ...
	 ci->listval[n + 1] = (char *) NULL;
      ...
      break;
   default:
      fprintf(stderr, _("internal error: set_conf(): strange type %d\n"),
	      ci->type);
      break;
   }
